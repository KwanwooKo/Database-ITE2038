...
lock_t *lock_acquire(int64_t table_id, pagenum_t page_id, int64_t key, int trx_id, int lock_mode) {
    ...
    if (deadlock_detection(trx_id, trx_id)) {
        trx_abort_not_with_latch(trx_id);
        pthread_mutex_unlock(&trx_manager_latch);
        return NULL;
    }
...
int trx_begin() {
    ...
    std::cout << "trx_begin: " << ret_trx_id << std::endl;
    write_general_log(ret_trx_id, BEGIN);
...
int trx_commit(int trx_id) {
    ...
    std::cout << "trx_commit: " << trx_id << std::endl;
    write_general_log(trx_id, COMMIT);
    flush_log_with_latch();
...
int trx_abort_not_with_latch(int trx_id) {
    ...
    for (auto iter : trx->roll_back) {
        ...
        char* old_img = new char[sp.size];
        char* new_img = new char[sp.size];
        memcpy(old_img, lp->page_body + sp.offset - 128, sp.size);
        memcpy(new_img, iter.second->original_value, sp.size);
        pagenum_t leaf_pagenum = iter.second->page_num;

        write_compensate_log(trx_id, iter.second->table_id, leaf_pagenum, sp.offset, sp.size, old_img, new_img);
        // write_update_log(trx_id, iter.second->table_id, leaf_pagenum, sp.offset, sp.size, old_img, new_img);
    ...
    write_general_log(trx_id, ROLLBACK);
    flush_log_with_latch();

    /* 이 과정은 db_update 에서 진행해야 될듯 */
    // trx_table.erase(trx_id);
    // delete trx;
    return trx_id;
}
...
int trx_abort(int trx_id) {
    pthread_mutex_lock(&trx_manager_latch);
    int ret = trx_abort_not_with_latch(trx_id);
    pthread_mutex_unlock(&trx_manager_latch);
    return ret;
}
