...
std::map<int64_t, int64_t> table_list;
std::map<int, pagenum_t> root_pagenum_list;

int64_t parse_path_name(const char* pathname) {
    ...
    int64_t ret = stoi(path);

    return ret;
}


/* map 써서 table_id 넣으면 fd 나오게 처리하자 */
int64_t open_table(const char * pathname) {
    /* 파싱하고 나서 숫자 이미 있는지 확인 */
    int64_t table_id = parse_path_name(pathname);
    if (table_list.find(table_id) != table_list.end()) {
        // std::cout << "[DEBUG] file already opened" << std::endl;
        return table_id;
    }
    ...
    table_list[table_id] = unique_id;
    // std::cout << "[DEBUG] first file opened" << std::endl;
    Header_page hp;
    buffer_read_page(table_id, 0, (page_t *)&hp);
    root_pagenum_list[table_id] = hp.root_page_number;  
...
int init_db(int buf_num, int flag, int log_num, char* log_path, char* logmsg_path) {
    table_list.clear();
    buffer_init(buf_num);
    int ret_flag = init_log(flag, log_num, log_path, logmsg_path);
    if (flag != 0) return ret_flag;
    maximum_fdlists_size = 20;
    root_pagenum_list.clear();
    return 0;
}
...
int shutdown_db() {
    std::cout << "[DEBUG] shutdown_db call" << std::endl;
    buffer_destroy();
    shutdown_log();
    ...
    for (uint32_t i = 0; i < current_number_of_keys; i++) {
        ...
        if (key == sp.key) {
            ...
            if (lock == NULL) {
                ...
                log_t* log = trx->roll_back[key];
                char *old_val = new char[new_val_size];
                char* new_val = new char[new_val_size];
                /* log 가 있으면 그냥 그 값 저장하면 돼 */
                if (log != NULL) {
                    memcpy(old_val, log->original_value, sp.size);
                }
                /* log 가 없다는 소리는 현재 value 를 저장 */
                else {
                    /* 현재 value 를 저장해야 돼 */
                    memcpy(old_val, lp->page_body + sp.offset - 128, sp.size);
                }
                memcpy(new_val, value, new_val_size);
                delete old_val;
                delete new_val;
            ...
            *old_val_size = new_val_size;
            ...
            int64_t page_LSN = write_update_log(trx_id, table_id, leaf_pagenum, sp.offset, new_val_size, old_val, new_val);
            std::cout << "update's page_LSN: " << page_LSN << std::endl;
            lp->page_LSN = page_LSN;
