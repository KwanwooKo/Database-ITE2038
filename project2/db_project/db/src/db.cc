...
int64_t open_table(const char * pathname) {
    ...
    Header_page hp;
    file_read_page(unique_id, 0, (page_t *)&hp);
...
int db_insert(int64_t table_id, int64_t key, const char* value, uint16_t val_size) {
    ...
    file_read_page(table_id, 0, (page_t *) hp);
    ...
    */
    if (hp->root_page_number == 0) {
        ...
        file_read_page(table_id, root_pagenum, (page_t *)lp);
        lp->number_of_keys++;
        lp->amount_of_free_space -= (val_size + 12);
        Slot_page sp;
        sp.key = key;
        sp.size = val_size;
        sp.offset = PAGE_SIZE - val_size;
        memcpy(lp->page_body, &sp, 12);
        memcpy(lp->page_body + sp.offset - 128, value, val_size);
        file_write_page(table_id, 0, (page_t *)hp);
        file_write_page(table_id, root_pagenum, (page_t *)lp);
    ...
    */
    ...
    file_read_page(table_id, leaf_page_num, (page_t *)lp);

    /**
    * Case: leaf has room for key and pointer
    * order를 대체할 수 있는 걸 만들어야겠네 => amount of free space
    */
    if ((pagenum_t)val_size + (pagenum_t)12 < lp->amount_of_free_space) {
        free(ret_val_from_find);
        free(tmp_size_from_find);
        free(hp);
        free(lp);
...
int db_insert_into_leaf_page_after_splitting(int64_t table_id, pagenum_t root_page_num, pagenum_t leaf_page_num, int64_t key, const char* value, uint16_t val_size) {
    ...
    file_read_page(table_id, leaf_page_num, (page_t *)lp);
    /**
     * 이거 왜 이렇게 되는지 솔직히 잘 모르겠다
     * 그냥 file_alloc_page 하고 나면 leaf_page_num 이 뒤틀려;
     */
    Leaf_page *new_lp = (Leaf_page *) malloc(sizeof(Leaf_page));
    pagenum_t new_leaf_page_num = make_leaf_page(table_id);
    file_read_page(table_id, new_leaf_page_num, (page_t *)new_lp);
    ...
    for (int i = 0; i < lp->number_of_keys; i++) {
        memcpy(&sp_list[i], lp->page_body + 12*i, 12);
    }
    ...
    for (int i = 0; i < lp->number_of_keys; i++) {
        ...
        lp->amount_of_free_space -= (12 + sp_list[i].size);
    ...
    for (int i = 0; i < new_lp->number_of_keys; i++) {
        ...
        new_lp->amount_of_free_space -= (12 + new_sp_list[i].size);
     ...
     */
    for (int i = 0; i < lp->number_of_keys; i++) {
        memcpy(lp->page_body + 12*i, &sp_list[i], 12);
        memcpy(lp->page_body + sp_list[i].offset - 128, value_list[i], sp_list[i].size);
    }
    for (int i = 0; i < new_lp->number_of_keys; i++) {
        memcpy(new_lp->page_body + 12*i, &new_sp_list[i], 12);
    ...
    file_write_page(table_id, leaf_page_num, (page_t *)lp);
    file_write_page(table_id, new_leaf_page_num, (page_t *)new_lp);
...
int db_insert_into_leaf_page(int64_t table_id, pagenum_t leaf_page_num, int64_t key, const char* value, uint16_t val_size) {
    ...
    */
    ...
    for (uint32_t i = 0; i < current_number_of_keys; i++) {
        Slot_page sp;
        // sp offset == 128 + 12 * i
        memcpy(&sp, lp->page_body + 12*i, 12);
        sp_list[i] = sp;
    }
...
int db_insert_into_parent_page(int64_t table_id, pagenum_t left_page_num, pagenum_t right_page_num, int64_t key) {
    ...
    file_read_page(table_id, left_page_num, (page_t *)left_page);
    file_read_page(table_id, right_page_num, (page_t *)right_page);
    ...
    if (left_page->parent_page_number == 0) {
        ...
        file_write_page(table_id, left_page_num, (page_t *)left_page);
        file_write_page(table_id, right_page_num, (page_t *)right_page);
        file_write_page(table_id, 0, (page_t *) hp);
        free(parent);
        free(left_page);
        free(right_page);
...
int db_insert_into_internal_page(int64_t table_id, pagenum_t parent_page_num, int left_index, int64_t key, pagenum_t right_page_num) {
    ...
    file_read_page(table_id, parent_page_num, (page_t *) parent_page);
    ...
    file_write_page(table_id, parent_page_num, (page_t *) parent_page);
    free(parent_page);
    return 0;
}
...
int db_insert_into_internal_page_after_splitting(int64_t table_id, pagenum_t parent_page_num, int left_index, int64_t key, pagenum_t right_page_num) {
    ...
    file_read_page(table_id, parent_page_num, (page_t *) old_page);
...
//    printf("left_index: %d\n", left_index);
    ...
    file_read_page(table_id, new_internal_page_num, (page_t *) new_page);
...
//        file_read_page(table_id, old_page->entry[j].page_number, (page_t *) cp);
//        cp->parent_page_number = new_internal_page_num;
//        file_write_page(table_id, old_page->entry[j].page_number, (page_t *) cp);
//        free(cp);
    }
    /**
    ...
    file_read_page(table_id, new_page->left_most_page_number, (page_t *) left_most_page);
    left_most_page->parent_page_number = new_internal_page_num;
    file_write_page(table_id, new_page->left_most_page_number, (page_t *) left_most_page);
    ...
    for (++i, j = 0; i < 249; i++, j++) {
        ...
        file_read_page(table_id, new_page->entry[j].page_number, (page_t *)cp);
        cp->parent_page_number = new_internal_page_num;
        file_write_page(table_id, new_page->entry[j].page_number, (page_t *) cp);
        free(cp);
    }
...
//    }
    ...
    file_write_page(table_id, parent_page_num, (page_t *) old_page);
    file_write_page(table_id, new_internal_page_num, (page_t *) new_page);
    /**
     * free memory
     */
    free(old_page);
 ...
 * 얘 무조건 leaf_page 다
 * Internal_page 는 val_size 정보가 없어
 */
int db_find(int64_t table_id, int64_t key, char* ret_val, uint16_t* val_size) {
    ...
    file_read_page(table_id, leaf_pagenum, (page_t *)lp);

    // store the current_number_of_keys
    uint32_t current_number_of_keys = lp->number_of_keys;
    Slot_page sp;
//    std::cout << "current_number_of_keys: " << current_number_of_keys << '\n';
//    std::cout << "leaf_pagenum: " << leaf_pagenum << '\n';
...
//    pagenum_t leaf_page_num = find_leaf_page(table_id, key);
    ...
    file_read_page(table_id, page_num, (page_t *) page);
    ...
    file_read_page(table_id, ((Internal_page *)page)->parent_page_number, (page_t *) parent);
    ...
    file_read_page(table_id, neighbor_page_num, (page_t *) neighbor);
...
pagenum_t db_delete_parent_entry(int64_t table_id, pagenum_t page_num, int k_prime_index) {
    ...
    if (page->parent_page_number == 0) {
        file_read_page(table_id, root_pagenum_list[table_id], (page_t *) parent);
        parent->number_of_keys--;
        for (i = k_prime_index; i < parent->number_of_keys; i++) {
            parent->entry[i] = parent->entry[i + 1];
        }
        file_write_page(table_id, root_pagenum_list[table_id], (page_t *) parent);
        free(page);
        free(neighbor);
        free(parent);
        return db_adjust_root(table_id);
    ...
    file_write_page(table_id, page_num, (page_t *) page);
    ...
    neighbor_index = get_neighbor_index(table_id, page_num);
    k_prime_index = neighbor_index == -1 ? 0 : neighbor_index;
    file_read_page(table_id, page->parent_page_number, (page_t *) parent);
    k_prime = parent->entry[k_prime_index].key;
...
//            printf("delete func: found key!\n");
    ...
    lp->number_of_keys--;
    lp->amount_of_free_space += (sp_list[deletion_index].size + 12);
...
pagenum_t db_adjust_root(int64_t table_id) {
    ...
    file_read_page(table_id, root_pagenum_list[table_id], (page_t *) root);
    /**
    ...
    if (root->is_leaf == 0) {
        ...
        file_free_page(table_id, root_pagenum_list[table_id]);
        root_pagenum_list[table_id] = 0;
        file_read_page(table_id, 0, (page_t *) hp);
        hp->root_page_number = 0;
        new_root_page_num = 0;
...
int get_neighbor_index(int64_t table_id, pagenum_t page_num) {
    ...
    Leaf_page *lp = (Leaf_page *) malloc(sizeof(Leaf_page));
    Internal_page *parent = (Internal_page *) malloc(sizeof(Internal_page));
    file_read_page(table_id, page_num, (page_t *) lp);
    file_read_page(table_id, lp->parent_page_number, (page_t *) parent);
...
pagenum_t merge_leaf_pages(int64_t table_id, pagenum_t leaf_page_num, pagenum_t neighbor_page_num, int neighbor_index, int64_t k_prime) {
    ...
    file_read_page(table_id, leaf_page_num, (page_t *) n);
    file_read_page(table_id, neighbor_page_num, (page_t *) neighbor);
    ...
    for (i = 0; i < n->number_of_keys; i++) {
        memcpy(&n_sp_list[i], n->page_body + 12*i, 12);
    ...
    for (i = 0; i < neighbor->number_of_keys; i++) {
        memcpy(&neighbor_sp_list[i], neighbor->page_body + 12*i, 12);
    ...
    for (i = neighbor_insertion_index, j = 0; j < n->number_of_keys; i++, j++) {
        ...
        neighbor->amount_of_free_space -= (n_sp_list[j].size + 12);
    }

    neighbor->right_sibling_page_number = n->right_sibling_page_number;

    for (i = 0; i < neighbor->number_of_keys; i++) {
        memcpy(neighbor->page_body + 12*i, &neighbor_sp_list[i], 12);
        memcpy(neighbor->page_body + neighbor_sp_list[i].offset - 128, neighbor_value_list[i], neighbor_sp_list[i].size);
    }

    file_write_page(table_id, neighbor_page_num, (page_t *) neighbor);
    file_free_page(table_id, leaf_page_num);
...
pagenum_t merge_internal_pages(int64_t table_id, pagenum_t page_num, pagenum_t neighbor_page_num, int neighbor_index, int64_t k_prime) {
    ...
    file_read_page(table_id, page_num, (page_t *) n);
    file_read_page(table_id, neighbor_page_num, (page_t *) neighbor);
    ...
    Internal_page *page = (Internal_page *) malloc(sizeof(Internal_page));
    file_read_page(table_id, n->left_most_page_number, (page_t *) page);
    page->parent_page_number = neighbor_page_num;
    file_write_page(table_id, n->left_most_page_number, (page_t *) page);

    for (i = 0; i < neighbor->number_of_keys; i++) {
        file_read_page(table_id, neighbor->entry[i].page_number, (page_t *) page);
        page->parent_page_number = neighbor_page_num;
        file_write_page(table_id, neighbor->entry[i].page_number, (page_t *) page);
    }
    /**
     * 이거 free 하는게 맞나 의문
     */
...
pagenum_t redistribute_leaf_pages(int64_t table_id, pagenum_t leaf_page_num, pagenum_t neighbor_page_num, int neighbor_index, int k_prime_index, int64_t k_prime) {
    ...
    if (neighbor_index != -1) {
        ...
        n->amount_of_free_space -= (n_sp_list[0].size + 12);
        neighbor->amount_of_free_space += (n_sp_list[0].size + 12);
        ...
        parent->entry[k_prime_index].key = neighbor_sp_list[0].key;
        ...
        n->amount_of_free_space -= (n_sp_list[n->number_of_keys].size + 12);
        neighbor->amount_of_free_space += (n_sp_list[n->number_of_keys].size + 12);
    ...
    for (i = 0; i < n->number_of_keys + 1; i++) {
        memcpy(n->page_body + 12*i, &n_sp_list[i], 12);
    ...
    file_write_page(table_id, parent_page_num, (page_t *) parent);
    file_write_page(table_id, leaf_page_num, (page_t *) n);
    file_write_page(table_id, neighbor_page_num, (page_t *) neighbor);
...
pagenum_t redistribute_internal_pages(int64_t table_id, pagenum_t internal_page_num, pagenum_t neighbor_page_num, int neighbor_index, int k_prime_index, int64_t k_prime) {
    ...
    Internal_page *neighbor = (Internal_page *) malloc(sizeof(Internal_page));
    Internal_page *parent = (Internal_page *) malloc(sizeof(Internal_page));
    file_read_page(table_id, internal_page_num, (page_t *) n);
    file_read_page(table_id, neighbor_page_num, (page_t *) neighbor);
    parent_page_num = n->parent_page_number;
    file_read_page(table_id, parent_page_num, (page_t *) parent);
    ...
    if (neighbor_index != -1) {
        ...
        file_read_page(table_id, n->left_most_page_number, (page_t *) tmp);
        tmp->parent_page_number = internal_page_num;
        file_write_page(table_id, n->left_most_page_number, (page_t *) tmp);
        ...
        file_read_page(table_id, n->entry[n->number_of_keys].page_number, (page_t *) tmp);
        tmp->parent_page_number = internal_page_num;
        ...
        file_write_page(table_id, n->entry[n->number_of_keys].page_number, (page_t *) tmp);
        free(tmp);
    }
    ...
    file_write_page(table_id, internal_page_num, (page_t *) n);
    file_write_page(table_id, neighbor_page_num, (page_t *) neighbor);
    file_write_page(table_id, parent_page_num, (page_t *) parent);
...
int db_scan(int64_t table_id, int64_t begin_key, int64_t end_key, std::vector<int64_t>* keys, std::vector<char*>* values, std::vector<uint16_t>* val_sizes) {
    ...
    while (1) {
        ...
        if (i != lp->number_of_keys) break;
            ...
            index = 0;
            file_read_page(table_id, lp->right_sibling_page_number, (page_t *) lp);
            free(sp_list);
...
pagenum_t find_leaf_page(int64_t table_id, int64_t key) {
    ...
    file_read_page(table_id, root_pagenum_list[table_id], (page_t *)ip);
    ...
    while (ip->is_leaf == 0) {
        ...
        file_read_page(table_id, pagenum, (page_t *)ip);
    }
    free(ip);
    return pagenum;
}
 ...
 * if root is leaf_page
 */
pagenum_t find_leaf_page(int64_t table_id) {
    Header_page* hp = (Header_page*)malloc(sizeof(Header_page));
    file_read_page(table_id, 0, (page_t *)hp);
    pagenum_t ret_pagenum = hp->root_page_number;
...
pagenum_t make_leaf_page(int64_t table_id) {
    ...
    pagenum_t lp_page_num = file_alloc_page(table_id);
    ...
    file_write_page(table_id, lp_page_num, (page_t *)lp);
...
pagenum_t make_internal_page(int64_t table_id) {
    ...
    file_write_page(table_id, root_page_num, (page_t *) new_ip);
